             ...
             node.$tree_link.css({
               ...
               node.$tree_link.on('click', function (e) {
                   if (!$(e.target).hasClass('octicon')) {
                       // Ensure toolbar appears for the selected node
                       if (cur_tree.toolbar) {
                           cur_tree.selected_node = node;
                           cur_tree.toolbar.show();
                       }
              
                       // Prevent self-duplication and check if it has children
                       if (node.expandable) {
                           cur_tree.load_children(node); // Load only if the node has children
                       }
   ...
   filters: [
       ...
       },
       {
           ...
           get_query: function () {
               var me = frappe.treeview_settings["Task"];
               var project = me.page.fields_dict.project.get_value();
               var selectedTask = me.page.fields_dict.task.get_value(); // Get selected task
               var args = [["Task", "is_group", "=", 1]];
               ...
               // Prevent task from treating itself as its parent when filtered
               if (selectedTask) {
                   args.push(["Task", "name", "!=", selectedTask]); // Exclude itself
               }
